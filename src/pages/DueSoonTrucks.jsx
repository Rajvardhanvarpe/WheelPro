...
import { getDueSoonTrucks } from '../services/dataService';
...
const DueSoonTrucks = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                const data = await getDueSoonTrucks();
                setDueSoon(data);
            } catch (error) {
                console.error(error);
            } finally {
    ...
    return (
        <div className="dark:bg-background-dark dark:text-gray-100 min-h-screen flex flex-col pb-24">
            <div className="flex items-center bg-transparent p-4 pb-2 justify-between sticky top-0 z-50 bg-[#EAEFEF]/80 dark:bg-background-dark/80 backdrop-blur-sm">
                <div className="text-workshop-dark dark:text-gray-100 flex size-12 shrink-0 items-center justify-start">
                    <span onClick={() => navigate(-1)} className="material-symbols-outlined text-2xl cursor-pointer">arrow_back_ios</span>
                </div>
                <h2 className="text-workshop-dark dark:text-white text-lg font-bold leading-tight tracking-[-0.015em] flex-1 text-center">Due Soon ({filteredDueSoon.length})</h2>
            ...
            <div className="px-4 pt-4 pb-2">
                <p className="text-sm text-gray-500 font-bold">Trucks due for alignment within 7 days.</p>
            </div>

            <div className="px-4 pb-6">
                {filteredDueSoon.length === 0 ? (
                    <p className="text-center text-gray-500 py-10 bg-white dark:bg-gray-800 rounded-xl shadow-sm">
                        {searchQuery ? `No trucks found matching "${searchQuery}"` : 'No trucks due soon.'}
                    </p>
                ) : (
                    <div className="flex flex-col gap-4">
                        {filteredDueSoon.map((item) => (
                            <div key={item.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-amber-100 dark:border-amber-900/40">
                                {/* Top row: Owner Name + badge + WhatsApp */}
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-2 flex-1 min-w-0">
                                        ...
                                        <span className="bg-amber-500 text-white px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider shrink-0">{getDaysRemaining(item.nextDueDate)}</span>
                                    </div>
                                    {/* WhatsApp Buttons */}
                                ...
                                <div className="grid grid-cols-2 gap-2 mb-3">
                                    ...
                                    <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg p-2.5">
                                        <p className="text-[10px] font-bold text-amber-500 uppercase tracking-wider mb-0.5">Due Date</p>
                                        <p className="text-sm font-black text-amber-600 dark:text-amber-400 flex items-center gap-1">
                                            <span className="material-symbols-outlined text-sm">event</span>
                                            {item.nextDueDate || 'Unknown'}
                                        </p>
                                    </div>
                                    <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2.5">
